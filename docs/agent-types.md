...
```javascript
Task({ subagent_type: "statusline-setup", prompt: "Set up a status line showing git branch and node version" })
```
...
| `refactor:simplifier` | Code simplification and clarity improvements post-refactor |
| `refactor:security-review` | Security regression review for refactored code |
