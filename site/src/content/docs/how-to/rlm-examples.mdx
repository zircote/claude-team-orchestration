---
title: "RLM Pattern Examples"
description: "Practical example prompts for every RLM mode."
---

Practical example prompts for every RLM mode. Copy, adapt the file path, and paste into Claude Code.

> **Prerequisite:** The `swarm` plugin must be installed and agent teams enabled. See [Getting Started](/getting-started/getting-started/).
...
> See [How RLM processes logs and prose](/concepts/concepts/#basic-logs-and-prose) for what happens internally.
...
> See [How RLM processes source code](/concepts/concepts/#source-code) for what happens internally. See [Analysis Focus Options](/reference/reference/#rlm-analysis-focus-options) for steering analyst priorities.
...
> See [How RLM processes CSV/TSV](/concepts/concepts/#csvtsv) for what happens internally. See [What Data Analysts Report](/reference/reference/#rlm-data-analyst-output) for the standard output format.
...
> See [How RLM processes JSON/JSONL](/concepts/concepts/#jsonjsonl) for what happens internally.
...
> **Skill reference:** See [`skills/jsonl-log-analyzer/SKILL.md`](/skills/jsonl-log-analyzer/) for the full procedure.
...
> See [How JSONL Log Analysis works](/concepts/concepts/#jsonl-log-analysis) for what happens internally. See [Standard vs JSONL Log Analyzer](/reference/reference/#standard-vs-jsonl-log-analyzer) for when to use each mode.
...
> See [How RLM processes directories](/concepts/concepts/#directory-and-multi-type-analysis) for what happens internally.
...
> See [How RLM processes directories](/concepts/concepts/#directory-and-multi-type-analysis) for what happens internally. See [Cross-File Analysis](/concepts/concepts/#cross-file-analysis) for what multi-type analysis catches that single-file cannot.
...
- [RLM Pattern Skill](/skills/rlm-pattern/) — Full technical reference
- [RLM Concepts](/concepts/concepts/#rlm-content-processing) — How content processing works internally
- [RLM Reference Tables](/reference/reference/#rlm-analysis-focus-options) — Analysis options, feature comparison, data output format
- [Content-Aware RLM Design](/design/content-aware-rlm/) — Design decisions for type detection and routing
- [Multi-File RLM Design](/design/multi-file-rlm/) — Design decisions for directory analysis
- [Agent Types](/reference/agent-types/) — All available analyst agents
- [Orchestration Patterns](/how-to/patterns/) — When to use RLM vs other patterns
