...
**Related skills:**
- [RLM Pattern](../rlm-pattern/SKILL.md) — Core chunked analysis pattern this skill builds on
- [Orchestration Patterns](../orchestration-patterns/SKILL.md) — Pattern 7 (RLM) and team lifecycle
...
| JSON array (not line-delimited) | No — use [Content-Aware JSON RLM](../rlm-pattern/SKILL.md) |
| CSV/TSV data files | No — use [Content-Aware CSV RLM](../rlm-pattern/SKILL.md) |
...
> **Template substitution:** The `{field}` tokens in recipe templates (e.g. `{level_field}`, `{timestamp_field}`) are **placeholder names** — replace them with the actual discovered field path before generating the recipe. For example, if the discovered timestamp field is `meta.ts`, then `.{timestamp_field}` becomes `.meta.ts`. The `{key: value}` jq object constructors in the same recipe (e.g. `{ts: .meta.ts}`) are real jq syntax and must not be substituted.
