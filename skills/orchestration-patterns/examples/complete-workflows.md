...
Task({
  ...
  SendMessage({ to: "team-lead", message: "Your findings here", summary: "Security review findings" })`,
...
// === STEP 5: Cleanup ===
SendMessage({ to: "security", message: { type: "shutdown_request", reason: "Review complete" } })
SendMessage({ to: "quality", message: { type: "shutdown_request", reason: "Review complete" } })
SendMessage({ to: "arch", message: { type: "shutdown_request", reason: "Review complete" } })
// Wait for approvals...
TeamDelete()
```
...
2. Find a task that is:
   ...
   - not blocked
3. If found:
   - Claim and start it: TaskUpdate({ taskId: "X", owner: "YOUR_NAME", status: "in_progress" })
   - Do the review work
   - Complete it: TaskUpdate({ taskId: "X", status: "completed" })
   - Send findings to team-lead via SendMessage
   - Go back to step 1
4. If no tasks available, send team-lead a message that you have finished all work and stop.
...
SendMessage({ to: "analyst-1", message: { type: "shutdown_request", reason: "Done" } })
SendMessage({ to: "analyst-2", message: { type: "shutdown_request", reason: "Done" } })
SendMessage({ to: "analyst-3", message: { type: "shutdown_request", reason: "Done" } })
SendMessage({ to: "synthesizer", message: { type: "shutdown_request", reason: "Done" } })
// Wait for approvals...
TeamDelete()
```
...
SendMessage({ to: "code-analyst-1", message: { type: "shutdown_request", reason: "Done" } })
SendMessage({ to: "code-analyst-2", message: { type: "shutdown_request", reason: "Done" } })
SendMessage({ to: "code-analyst-3", message: { type: "shutdown_request", reason: "Done" } })
SendMessage({ to: "data-analyst-1", message: { type: "shutdown_request", reason: "Done" } })
SendMessage({ to: "data-analyst-2", message: { type: "shutdown_request", reason: "Done" } })
SendMessage({ to: "json-analyst-1", message: { type: "shutdown_request", reason: "Done" } })
SendMessage({ to: "synthesizer", message: { type: "shutdown_request", reason: "Done" } })
// Wait for approvals...
TeamDelete()
```
